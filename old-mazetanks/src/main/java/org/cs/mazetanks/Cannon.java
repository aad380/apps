...
class Cannon extends GameObject implements GuidedDevice {
    ...
    public Cannon(int x, int y, int direction, GameManager manager, Player player) {
        ...
        bigBullets = 20;
        nukeBullets = 3;
        currentWeapon = GuidedDevice.BULLET;
    }
...
// + GuidedDevice interface
    ...
    public void selectBullet() {
        if (state != KILLED_STATE) {
            if (currentWeapon != GuidedDevice.BULLET) {
                currentWeapon = GuidedDevice.BULLET;
    ...
    public void selectBigBullet() {
        if (state != KILLED_STATE) {
            if (currentWeapon != GuidedDevice.BIG_BULLET) {
                currentWeapon = GuidedDevice.BIG_BULLET;
                lastGunReloadTime = System.currentTimeMillis();
            }
        }
    }
    ...
    public void selectNukeBullet() {
        if (state != KILLED_STATE) {
            if (currentWeapon != GuidedDevice.NUKE_BULLET) {
                currentWeapon = GuidedDevice.NUKE_BULLET;
                lastGunReloadTime = System.currentTimeMillis();
            }
        }
    }
...
// - GuidedDevice interface
    ...
    public void step() {
        ...
        player.step();        // hook for CannonDriver
        if (state == KILLED_STATE) {
            return;
        }
        if (fireFlag) {
