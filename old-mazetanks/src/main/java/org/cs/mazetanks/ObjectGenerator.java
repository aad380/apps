...
public class ObjectGenerator {
    ...
    public Tank makeTank(Player player, int tryCounter) {
        ...
        if (visableObjectMode) {
            tank = new VisibleTank(-1, -1, GameObject.NO_DIRECTION, manager, player, (GameSpaceCanvas) space);
        } else {
    ...
    public BigBullet makeBigBullet(Player player, int tryCounter) {
        ...
        if (space.getObject(x, y) instanceof Space) {
            ...
            if (visableObjectMode) {
                bullet = new VisibleBigBullet(x, y, GameObject.NO_DIRECTION, manager, player, (GameSpaceCanvas) space);
            } else {
    ...
    public BulletBonus makeBulletBonus(int tryCounter) {
        ...
        if (visableObjectMode) {
            bonus = new VisibleBulletBonus(-1, -1, manager, (GameSpaceCanvas) space);
        } else {
    ...
    public BigBulletBonus makeBigBulletBonus(int tryCounter) {
        ...
        if (visableObjectMode) {
            bonus = new VisibleBigBulletBonus(-1, -1, manager, (GameSpaceCanvas) space);
        } else {
    ...
    public NukeBulletBonus makeNukeBulletBonus(int tryCounter) {
        ...
        if (visableObjectMode) {
            bonus = new VisibleNukeBulletBonus(-1, -1, manager, (GameSpaceCanvas) space);
        } else {
    ...
    public ShieldBonus makeShieldBonus(int tryCounter) {
        ...
        if (visableObjectMode) {
            bonus = new VisibleShieldBonus(-1, -1, manager, (GameSpaceCanvas) space);
        } else {
