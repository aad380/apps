...
class Tank extends Cannon implements GuidedDevice {
	...
	private int	stepState = GuidedDevice.STOP;
	...
	public Tank (int x, int y, int direction, GameManager manager, Player player) {
		super (x, y, 3, 3, direction, 100, 20, 10, GuidedDevice.BULLET, 3, manager, player);
...
// + org.cs.GuidedDevice interface
	...
	public void goForward () {
		if (state != KILLED_STATE) {
			stepState = GuidedDevice.FORWARD;
			state = STOP_STATE;
		}
	}
	...
	public void goBackward () {
		if (state != KILLED_STATE) {
			stepState = GuidedDevice.BACKWARD;
			state = STOP_STATE;
		}
	}
...
// - org.cs.GuidedDevice interface
	...
	public void step () {
		...
		player.step ();		// hook for org.cs.TankDriver
		if (state == KILLED_STATE) {
			return;
		}
		if (state == STOP_STATE) {
			switch (stepState) {
			case GuidedDevice.FORWARD:
				move (direction);
				stepState = GuidedDevice.STOP;
				break;
			case GuidedDevice.BACKWARD:
